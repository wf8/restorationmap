<?php 
// 10/19/2011 modified for use with restoration map - server doesn't have dbase module installed
//
// modified to use xbase instead of php dbase module
// if writing/reading DBF file will need to use xbase
//
// added saveSimpleShapefile for writing simple SHP & SHX files containing single polygon (doesn't depend on xbase)
//
// edited _openDBFFile and _closeDBFFile to handle pointer to DBFFile with id=0 (set by xbase) - this will still need
// work to be fully functional
//
// required to use converted dbase api
require_once "phpxbase/api_conversion.php";
  ...
  class ShapeFile {
    ...
    // function to write SHP & SHX (no DBF) for single polygon
    // used by restoration map
    // does not depend on xbase
    function saveSimpleShapefile($FileName = NULL) {
      if ($FileName != NULL) $this->FileName = $FileName;

      if ( ($this->_openSHPFile(TRUE))  && ($this->_openSHXFile(TRUE))) {
 		
      		// first write headers
			$this->_saveHeaders();	       
       		// then write records to SHP file
        	$offset = 50;
     		if (is_array($this->records) && (count($this->records) > 0)) {
        		reset($this->records);
        		// go through each record
        		while (list($index, $record) = each($this->records)) {
					// write record headings
					$record->recordNumber = $index + 1;
					fwrite($this->SHPFile, pack("N", $record->recordNumber));
				    fwrite($this->SHPFile, pack("N", $record->getContentLength()));
				    fwrite($this->SHPFile, pack("V", $record->shapeType));
 					fwrite($this->SHPFile, pack("dddd", $record->SHPData["xmin"], $record->SHPData["ymin"], $record->SHPData["xmax"], $record->SHPData["ymax"]));
      				fwrite($this->SHPFile, pack("VV", $record->SHPData["numparts"], $record->SHPData["numpoints"] ));
			      	for ($i = 0; $i < $record->SHPData["numparts"]; $i++) {
			        	fwrite($this->SHPFile, pack("V", count($record->SHPData["parts"][$i])));
			      	}
			      	// done writing record header
			      	// no write record part data
			      	reset($record->SHPData["parts"]);
			      	foreach ($record->SHPData["parts"] as $partData) {
			        	reset($partData["points"]);
			        	while (list($pointIndex, $pointData) = each($partData["points"])) {
			        		// write each point
				        	fwrite($this->SHPFile, packDouble($pointData["x"]));
      						fwrite($this->SHPFile, packDouble($pointData["y"]));
			        	}
			      	}
					// done writing this record to SHP file
					// now write this record to SHX file
					fwrite($this->SHXFile, pack("N", $offset));
			    	fwrite($this->SHXFile, pack("N", $record->getContentLength()));
		    		$offset += (4 + $record->getContentLength());
		    		// done writing this record to SHX file
		   		}
      		}    
	        $this->_closeSHPFile();
	        $this->_closeSHXFile();
    ...
    function addRecord($record) {
      ...
      // added 10/28/2011
      if (($this->boundingBox["xmin"] == 0) || ($record->SHPData["xmin"] < $this->boundingBox["xmin"])) $this->boundingBox["xmin"] = $record->SHPData["xmin"];
      if (($this->boundingBox["ymin"] == 0) || ($record->SHPData["ymin"] < $this->boundingBox["ymin"])) $this->boundingBox["ymin"] = $record->SHPData["ymin"];
      if (($this->boundingBox["xmax"] == 0) || ($record->SHPData["xmax"] > $this->boundingBox["xmax"])) $this->boundingBox["xmax"] = $record->SHPData["xmax"];
      if (($this->boundingBox["ymax"] == 0) || ($record->SHPData["ymax"] > $this->boundingBox["ymax"])) $this->boundingBox["ymax"] = $record->SHPData["ymax"];
    ...
    function _deleteRecordFromDBF($index) {
      if (@xbase_delete_record($this->DBFFile, $index)) {
        @xbase_pack($this->DBFFile);
      }
    }
    ...
    function _saveRecords() {
      ...
      if (!($this->DBFFile = @xbase_create(str_replace('.*', '.dbf', $this->FileName), $this->DBFHeader))) {
    ...
    function _openDBFFile($toWrite = false) {    	
      ...
      if (($toWrite) && (!file_exists(str_replace('.*', '.dbf', $this->FileName)))) {
        if (!@xbase_create(str_replace('.*', '.dbf', $this->FileName), $this->DBFHeader)) {
          return $this->setError(sprintf("It wasn't possible to create the DBase file '%s'", str_replace('.*', '.dbf', $this->FileName)));
        }
      }
      if ($checkFunction(str_replace('.*', '.dbf', $this->FileName))) {     	
        $this->DBFFile = xbase_open(str_replace('.*', '.dbf', $this->FileName), ($toWrite ? 2 : 0));
     //   if (!$this->DBFFile) {
  ...
  class ShapeRecord {
    ...
    function _loadDBFData() { 	
      $this->DBFData = @xbase_get_record_with_names($this->DBFFile, $this->recordNumber);  
      unset($this->DBFData["deleted"]);        
    }
        
    function _saveDBFData() {
      unset($this->DBFData["deleted"]);
      if ($this->recordNumber <= xbase_numrecords($this->DBFFile)) {
        if (!xbase_replace_record($this->DBFFile, array_values($this->DBFData), $this->recordNumber)) {
          $this->setError("I wasn't possible to update the information in the DBF file.");
        }
      } else {
        if (!xbase_add_record($this->DBFFile, array_values($this->DBFData))) {
